...
# 데이터 생성 (고정된 탭 및 시드에서 다양한 초기값 및 탭 사용으로 확장)
num_samples = 10000
lfsr_length = 8
X = []
y = []
...
model = Sequential()
model.add(LSTM(units=units_1, activation='relu', input_shape=(lfsr_length, 1), return_sequences=True, kernel_regularizer=l2(0.01)))  # 정규화 강도 증가
...
# 테스트 데이터에 대한 주기 예측
test_stream = lfsr(0xDEADBEEFCAFEBABE, [0, 1, 2, 7, 15], lfsr_length)  # 임의의 테스트 데이터
